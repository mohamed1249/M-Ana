...
def read_data(file_path, **kwargs)-> pd.DataFrame:
...
def read_images_tf(folder_path, labels='inferred', label_mode='binary', image_size=[128, 128], interpolation='nearest', batch_size=64, shuffle=True,) -> Any:
    """Reads image dataset using TensorFlow and returns as a tf.data.Dataset."""

    from tensorflow.keras.preprocessing import image_dataset_from_directory
    import tensorflow as tf
    from ..modeling.data_preprocessing import image_to_float
...
def read_from_database(database_url, query) -> pd.DataFrame:
...
def read_from_html(url, table_index=0) -> pd.DataFrame:
